...
- (void)createContentInForm:(FORM_REC *)form
{
   ...
   form->labelField = [self createLabelInForm:form version:"Title:"];

   form->popUpButton = [self createPopUpInForm:form];
}
...
- (NSTextField *)createLabelInForm:(FORM_REC *)form version:(char *)verStr
{
   ...
   [cell setStringValue:[NSString stringWithFormat:@"Ver: [%s]", verStr]];
   // [cell setAlignment:_oscontrol_text_alignment(ekLEFT)];
...
)
{
   ...
   if (form->DITL_handle && form->ditl_def)  {
      ...
      [newWin setDelegate:((NiblesTestAppDelegate *)[NSApp delegate]).windowFactory];
      ...
      for (index=0; index<=form->last_fldno; index++)  {
         ...
         if (f_ditl_def->i_type & editText)  {              /* If TE field */
            ...
            f_ditl_def->i_handle = (Handle) [appDelegate.firstFormHandler coreCreateEditFieldWithFrame:id_CocoaRect(newWin, tmpRect)
                                                                                                inForm:form];
            ...
            if (pureIType == (ctrlItem+btnCtrl))  {       /* Simple Button */
               f_ditl_def->i_handle = (Handle) [appDelegate.firstFormHandler coreCreateButtonWithFrame:tmpRect
                                                                                                inForm:form
                                                                                                 title:(NSString *)buttonTitle];
            }
